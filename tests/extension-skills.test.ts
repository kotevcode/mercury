...
describe("installExtensionSkills", () => {
	...
	it("skips extensions without skillDir", () => {
		installExtensionSkills(
			[makeMeta("no-skill", undefined)],
			globalDir,
			log,
		);
		expect(
			fs.existsSync(path.join(globalDir, "skills", "no-skill")),
		).toBe(false);
	});
	...
	it("handles multiple extensions", () => {
		...
		expect(
			fs.existsSync(
				path.join(globalDir, "skills", "ext-a", "SKILL.md"),
			),
		).toBe(true);
		expect(
			fs.existsSync(
				path.join(globalDir, "skills", "ext-b", "SKILL.md"),
			),
		).toBe(true);
	});
	...
	it("creates skills dir if it doesn't exist", () => {
		...
		installExtensionSkills(
			[makeMeta("test", skillDir)],
			freshGlobal,
			log,
		);
		expect(
			fs.existsSync(
				path.join(freshGlobal, "skills", "test", "SKILL.md"),
			),
		).toBe(true);
	});
...
describe("installBuiltinSkills", () => {
	it("copies built-in skill directories", () => {
		...
		expect(
			fs.existsSync(
				path.join(globalDir, "skills", "tasks", "SKILL.md"),
			),
		).toBe(true);
	});
	...
	it("skips non-directory entries", () => {
		...
		installBuiltinSkills(builtinDir, globalDir, log);
		expect(
			fs.existsSync(path.join(globalDir, "skills", "readme.txt")),
		).toBe(false);
	});
	...
	it("copies multiple built-in skills", () => {
		...
		for (const name of ["tasks", "roles", "config"]) {
			expect(
				fs.existsSync(
					path.join(globalDir, "skills", name, "SKILL.md"),
				),
			).toBe(true);
		}
	});
